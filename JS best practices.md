...
Все переменные, используемые в функции, должны быть объявлены локально

Локальные переменные должны объявляться при помощи ключевого слова `let`, иначе они станут глобальными.
...
## 6. Комментировать столько, сколько необходимою
